...
<body <?php body_class(); ?>>
    <!-- IMAGEN DESTACADA -->
    <?php  
        $portfolio = get_option('page_for_posts');
        $id_image = get_post_thumbnail_id($portfolio);
        $img = wp_get_attachment_url($id_image, 'full');
    ?>
    
    <header class="site-header" style="background-image:url(<?php echo $img; ?>)">
